...
- [ ] **Task 1:** Create cluster `Aura-Dev-Cluster` and notebook `1.0_lakehouse_platform_practice`.
- [ ] **Task 2:** Create schemas `aura_raw_external` (external location) and `aura_gold_managed` (managed).
...
- [ ] **Task 8:** Create `sales_summary` managed table in `aura_gold_managed`. Compare paths, drop both tables, and verify which files are deleted from storage.
...
1. Export your practice notebook(s) or SQL script as a source file (e.g., SQL or Python file).
2. Save them inside this `nico-project` directory (e.g., `practice/1.0_lakehouse_platform_practice.sql`).
...
1. Read your practice scripts/SQL files in this workspace.
